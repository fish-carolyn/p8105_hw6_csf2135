...
df_birthweight_analytic |> 
  ...
  count(`Dad's Race`, `Mom's Race`) |> 
  pivot_wider(names_from = `Dad's Race`, values_from = n) |>
  kableExtra::kable()
```
...
```{r}
fit = lm(bwt ~ wtgain + smoken + momage + gaweeks + mrace + frace + fincome, data = df_birthweight_analytic)
...
fit |> 
  broom::tidy() |> 
  mutate(term = 
           str_replace(term, "^frace", "Dad's race: "), 
         term = 
