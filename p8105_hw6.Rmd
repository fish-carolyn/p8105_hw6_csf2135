...
sim_estimates = weather_df |> 
  ...
  mutate(
    ...
    beta_product = map(models, ~ log(abs((coef(.x)["tmin"] * coef(.x)["prcp"]))))
  ) |> 
    unnest(c(rsquared, beta_product)) |>
  select(.id, rsquared, beta_product) 
```
...
plot_sim_estimates_r2 = sim_estimates  |> 
  ...
  labs(title = "R-squared values for simulated weather estimates", x = "Count", y = "R-squared value") +
    theme(
      panel.background = element_rect(fill = "grey95"), 
      axis.title = element_text(color = "black", size = 14), 
      axis.text = element_text(color = "black", size = 11), 
...
plot_sim_estimates_log = sim_estimates  |> 
  ...
  labs(title = "log(β̂1xβ̂2) values for simulated weather estimates", x = "Count", y = "log(β̂1xβ̂2)") +
    theme(
      panel.background = element_rect(fill = "grey95"), 
      axis.title = element_text(color = "black", size = 14), 
      axis.text = element_text(color = "black", size = 11), 
...
```
The estimated R-squared values for the simulated weather data have a left skew. The median of the data is at `r sim_estimates |> pull(beta_product) |> median()|> round(digits = 2)`. 
