...
fit = glm(outcome ~ victim_age + sex + race, data = baltimore_df |> filter(city_state == "Baltimore, MD") , 
          family = "binomial")
...
results_all = 
  ...
  mutate(
    models = map(data, 
                 ~ glm(outcome ~ victim_age + sex + race, 
                        data = ., family = "binomial")),
    tidy_summary = map(models, broom::tidy)
  ) %>%
  unnest(tidy_summary) %>% view()
  select(city_state, term, estimate) %>% 
  mutate(city_state = fct_reorder(city_state, estimate))
  
conf_int_all = homicide_df %>%
  nest(data = c(outcome, victim_age, sex, race)) %>%
  mutate(
    models = map(data, 
                 ~ glm(outcome ~ victim_age + sex + race, 
                        data = ., family = "binomial")),
    conf = map(models, confint)
  ) %>% 
  unnest(conf) %>% view()
  select(city_state, term, estimate) %>% 
