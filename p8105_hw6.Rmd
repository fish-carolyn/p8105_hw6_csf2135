...
sim_weather_cis |> select(rsquared_lower)
```

The 95% CI for R-square values for the relationship between tmax and tmin + prcp is: (`r sim_weather_cis |> select(rsquared_lower)` - `r sim_weather_cis |> select(rsquared_upper)`). 

The 95% CI for the  log(β̂1∗β2) is: (`r sim_weather_cis |> select(logbeta_lower)` - `r sim_weather_cis |> select(logbeta_upper)`). 
